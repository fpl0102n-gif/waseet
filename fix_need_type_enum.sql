...
-- 2. Standardize 'need_type' column (TEXT)

-- For Local: Add if missing (it was missing causing 42703 error)
ALTER TABLE public.local_medicine_requests 
ADD COLUMN IF NOT EXISTS need_type TEXT DEFAULT 'medicine';

-- For Foreign: Convert from ENUM to TEXT (it was enum causing 22P02 error)
ALTER TABLE public.foreign_medicine_requests 
ALTER COLUMN need_type TYPE TEXT USING need_type::text;

-- Ensure financial_ability is also TEXT (idempotent check)
ALTER TABLE public.local_medicine_requests 
