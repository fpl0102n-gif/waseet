...
import { getWaseetTemplate, getAlkhayrTemplate, getBloodTemplate, getExchangeTemplate, getAgentRegistrationTemplate, getAgentApprovedTemplate, getBloodDonorRegistrationTemplate, getBloodDriverRegistrationTemplate, getBloodDonorAcceptanceTemplate, getBloodDonationThankYouTemplate, getWithdrawalRequestTemplate, getWithdrawalProcessedTemplate } from "./templates.ts";
...
serve(async (req) => {
    ...
    try {
        ...
        console.log(`[DEBUG V8] Incoming Type: '${type}' (Length: ${type.length})`);
        const allowed = ['order', 'order_update', 'exchange_request', 'import_request', 'product_suggestion', 'agent_registration', 'agent_approved', 'blood_donor_registration', 'transport_volunteer', 'blood_donor_acceptance', 'blood_donation_thank_you', 'material_donation_update', 'withdrawal_request', 'withdrawal_processed'];
        console.log(`[DEBUG V8] Is Allowed? ${allowed.includes(type)}`);
        ...
        if (allowed.includes(type)) {
            ...
            if (type === 'order') {
                ...
                subject = "Withdrawal Request Received - Waseet";
                // Fallback to contact_info if email is not in the explicit field (for backward compat or if passed in contact_info)
                userEmail = record.email;
                if (!userEmail && record.contact_type === 'email' && record.contact_info && record.contact_info.includes('@')) {
                    userEmail = record.contact_info;
                    console.log(`[Email] Fallback: Using contact_info as email: ${userEmail}`);
                }

                html = getWithdrawalRequestTemplate({
                    phone: record.contact_info || "N/A",
                    amount: `${record.amount_da} DZD`,
                    method: record.payment_notes || "N/A"
                });
            } else if (type === 'withdrawal_processed') {
                const status = record.status || "Processed";
                subject = `Withdrawal Update: ${status} - Waseet`;

                // Fallback to contact_info if email is not in the explicit field
                userEmail = record.email;
                if (!userEmail && record.contact_type === 'email' && record.contact_info && record.contact_info.includes('@')) {
                    userEmail = record.contact_info;
                    console.log(`[Email] Fallback: Using contact_info as email: ${userEmail}`);
                ...
                console.log(`[Email DEBUG] Withdrawal Processed - Email: ${userEmail}, ID: ${record.id}`);

                html = getWithdrawalProcessedTemplate({
                    amount: `${record.amount_da} DZD`,
                    status: status,
        ...
        // 2️⃣ ALKHAYR – Humanitarian
        else if (['alkhayr_request', 'alkhayr_request_local', 'alkhayr_request_foreign', 'material_donation', 'transport_volunteer', 'material_donation_update'].includes(type)) {
            ...
            if (type.startsWith('alkhayr_request')) {
                ...
                console.log("[Alkhayr Debug] Record:", JSON.stringify(record));

                const needName = record.medicine_name || record.medicine_details || "Medical Assistance";

                // Financial Logic
                let financialStr = record.financial_ability || "N/A";
                if (record.financial_ability === 'partial') {
                    const amount = record.afford_amount || record.budget || record.offered_amount || "0";
                    financialStr = `Partial - ${amount} DZD`; // Requested format: "Partial - Price"
                }
                ...
                const sections = [
                    {
                        ...
                        items: [
                            { label: "Medication Name", value: record.medicine_name || "N/A" },
                            { label: "Description / Details", value: record.description || "No description provided." }
                        ]
                    },
                    {
                        title: "Requester Information",
                        items: [
                            { label: "Full Name", value: record.full_name || "N/A" },
                            { label: "Phone Number", value: record.phone_number || "N/A" },
                            { label: "Email", value: record.email || "N/A" }
                        ]
                    }
                ];
                ...
                const locationItems = [];
                if (isForeign) {
                    locationItems.push({ label: "Country", value: record.expected_country || "N/A" });
                    locationItems.push({ label: "City / Municipality", value: record.city || "N/A" });
                } else {
                    locationItems.push({ label: "Wilaya", value: record.wilaya || "N/A" });
                    locationItems.push({ label: "City / Municipality", value: record.city || "N/A" });
                }
                sections.push({ title: "Location", items: locationItems });

                const supportItems = [
                    { label: "Financial Capacity", value: financialStr },
                    { label: "Delivery", value: record.need_delivery ? "Yes" : "No" }
                ];
                sections.push({ title: "Support & Capacity", items: supportItems });

                const priorityItems = [
                    { label: "Vital Emergency", value: record.is_urgent ? "YES - CRITICAL" : "No" }
                ];
                sections.push({ title: "Priority Assessment", items: priorityItems });

                html = getAlkhayrTemplate({
                    title: isForeign ? "Foreign Humanitarian Medication Request" : "Local Humanitarian Medication Request",
                    subtitle: isForeign
                        ? ""
                        : "",
                    items: [],
            ...
            else if (type === 'material_donation_update') {
                const status = record.status;
                const note = (payload as any).admin_note || record.rejection_reason;

                console.log(`[Email] Material Update. Status: ${status}, Note: ${note}`);

                if (status === 'approved') {
                    subject = "Donation Offer Approved";
                    const items = [
                        { label: "Item", value: record.item_name || "Donation" },
                        { label: "Category", value: record.category || "General" },
                        { label: "Status", value: "Approved & Published" }
                    ];
                    if (note) items.push({ label: "Admin Message", value: note });

                    html = getAlkhayrTemplate({
                        title: "Offer Approved",
                        subtitle: "Your donation offer has been approved.",
                        items: items
                    });
                }
                else if (status === 'rejected') {
                    subject = "Update regarding your Donation";
                    html = getAlkhayrTemplate({
                        title: "Offer Update",
                        subtitle: "Thank you for your offer. Unfortunately, we cannot accept it at this time.",
                        items: [
                            { label: "Item", value: record.item_name || "Donation" },
                            { label: "Status", value: "Not Accepted" },
                            { label: "Reason", value: note || "N/A" }
                        ]
                    });
                }
                    ...
                    html = getAlkhayrTemplate({
                        title: "Impact Made!",
                        subtitle: "Your donation has been successfully completed.",
                        items: [
                            { label: "Item", value: record.item_name || "Donation" },
                            { label: "Status", value: "Completed" },
                            { label: "Message", value: note || "Your generosity has helped someone in need." }
                        ]
                    });
                }
                else if (status === 'pending') {
                    subject = "Donation Under Review";
                    html = getAlkhayrTemplate({
                        title: "Review in Progress",
                        subtitle: "Your donation offer is currently being reviewed by our team.",
                        items: [
                            { label: "Item", value: record.item_name || "Donation" },
                            { label: "Status", value: "Under Review" },
                            { label: "Message", value: note || "We will notify you once a decision is made." }
                        ]
                    });
                } else {
                    return new Response(JSON.stringify({ message: "No email triggered" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
                }
            }

        }
        else if (type === 'withdrawal_processed') {
            const { status, amount_da, admin_note, referral_code } = record;

            // Determine Subject and Message based on status
            subject = status === 'paid' ? "Payment Processed - Waseet" : "Withdrawal Request Received - Update";
            // User email should be in record.email if we saved it, or we rely on passed payload record having it
            // In AdminReferrals.tsx, we pass: record: { ...selectedRequest, status, admin_note }
            // selectedRequest now has 'email' because we added it to DB and fetched it via select('*')

            userEmail = record.email;

            // Use the template
            html = getWithdrawalProcessedTemplate({
                amount: `${amount_da} DZD`,
                status: status,
                note: admin_note,
        ...
        // 3️⃣ BLOOD DONATION
            ...
            return new Response(JSON.stringify({ message: `Unknown type (DEPLOY CHECK V5): ${type}` }), {
                headers: { ...corsHeaders, "Content-Type": "application/json" },
            });
        }
        ...
        console.log(`[Email DEBUG] HTML Length: ${html ? html.length : 0}`);
        if (!html) console.error(`[Email DEBUG] HTML IS EMPTY! Type: ${type}`);
