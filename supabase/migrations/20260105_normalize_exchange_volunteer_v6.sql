-- Migration: Normalize Exchange & Volunteers (V6 - Dynamic Policy Drop)
-- Objective: Enforce Enums and implement strict privacy, dynamically dropping ALL existing policies to prevent dependency errors.
...
-- DYNAMIC DROP: All Policies on 'exchange_requests'
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'exchange_requests' AND schemaname = 'public') LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.exchange_requests', r.policyname);
    END LOOP;
END $$;
...
-- DYNAMIC DROP: All Policies on 'transport_volunteers' (The ultimate fix)
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'transport_volunteers' AND schemaname = 'public') LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.transport_volunteers', r.policyname);
    END LOOP;
END $$;
