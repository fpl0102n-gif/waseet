...
serve(async (req) => {
    ...
    try {
        ...
        const allowed = ['order', 'order_update', 'exchange_request', 'import_request', 'product_suggestion', 'agent_registration', 'agent_approved', 'blood_donor_registration', 'transport_volunteer', 'blood_donor_acceptance', 'blood_donation_thank_you', 'material_donation_update', 'withdrawal_request'];
        ...
        // 2️⃣ ALKHAYR – Humanitarian
            ...
            if (type.startsWith('alkhayr_request')) {
                ...
                items.push({ label: "Financial Capacity", value: financialStr });
                items.push({ label: "Privacy", value: "Protected" });

                html = getAlkhayrTemplate({
                    title: "Medical Request Received", // Slightly clearer title
                    subtitle: "Your request has been securely recorded.",
                    items: items
                });
            }
        ...
        // 3️⃣ BLOOD DONATION
            ...
            userEmail = record.email; // Explicitly passed from frontend
